				...
				<div class="twec-event-details">
					<?php if ( $start_date ) : ?>
						<div class="twec-event-date-time">
							<strong><?php esc_html_e( 'Date & Time:', 'planit-event-manager' ); ?></strong>
							<?php
							try {
								// Handle timezone.
								...
								if ( $all_day ) {
									...
									echo ' <span class="twec-all-day">(' . esc_html__( 'All Day', 'planit-event-manager' ) . ')</span>';
								} else {
								...
								echo '<span class="twec-event-datetime-raw">' . esc_html( (string) $start_date );
								if ( $end_date ) {
									echo ' — ' . esc_html( (string) $end_date );
								}
								echo '</span>';
							}
							?>
					...
					<?php if ( $event_cost ) : ?>
						<div class="twec-event-cost">
							<strong><?php esc_html_e( 'Cost:', 'planit-event-manager' ); ?></strong>
					...
					if ( class_exists( 'TWEC_WooCommerce', false ) ) {
						echo TWEC_WooCommerce::get_list_ticket_cta_html( get_the_ID() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method returns escaped HTML.
					}
					?>
					
					<?php if ( $event_website ) : ?>
						<div class="twec-event-website">
							<strong><?php esc_html_e( 'Event Website:', 'planit-event-manager' ); ?></strong>
					...
					if ( $venue_id ) :
						...
						<div class="twec-event-venue">
							<strong><?php esc_html_e( 'Venue:', 'planit-event-manager' ); ?></strong>
					...
					if ( $organizer_id ) :
						...
						<div class="twec-event-organizer">
							<strong><?php esc_html_e( 'Organizer:', 'planit-event-manager' ); ?></strong>
					...
					<?php
					$twec_ical_export_url   = add_query_arg(
						array(
							'twec_export' => 'ical',
							'event_id'    => get_the_ID(),
							'_wpnonce'    => wp_create_nonce( 'twec_export_ical' ),
						),
						home_url()
					);
					$twec_google_export_url = add_query_arg(
						array(
							'twec_export' => 'google',
							'event_id'    => get_the_ID(),
							'_wpnonce'    => wp_create_nonce( 'twec_export_google' ),
						),
						home_url()
					);
					?>
					<div class="twec-event-export">
						<a href="<?php echo esc_url( $twec_ical_export_url ); ?>" class="twec-export-btn"><?php esc_html_e( 'Add to iCal', 'planit-event-manager' ); ?></a>
						<a href="<?php echo esc_url( $twec_google_export_url ); ?>" class="twec-export-btn" target="_blank" rel="noopener"><?php esc_html_e( 'Add to Google Calendar', 'planit-event-manager' ); ?></a>
					</div>
				</div>
					...
					<footer class="entry-footer">
						<?php if ( $categories ) : ?>
							<div class="twec-event-categories">
								<strong><?php esc_html_e( 'Categories:', 'planit-event-manager' ); ?></strong>
						...
						<?php if ( $tags ) : ?>
							<div class="twec-event-tags">
								<strong><?php esc_html_e( 'Tags:', 'planit-event-manager' ); ?></strong>
								<?php foreach ( $tags as $event_tag ) : // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited -- Local loop variable ?>
									<a href="<?php echo esc_url( get_term_link( $event_tag ) ); ?>"><?php echo esc_html( $event_tag->name ); ?></a>
								<?php endforeach; ?>
							</div>
						<?php endif; ?>
					</footer>
