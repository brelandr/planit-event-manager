...
if ( ! isset( $twec_list_show_ticket_ctas ) ) {
			...
			while ( $events_query->have_posts() ) :
				...
				<article class="twec-event-item">
					...
					<div class="twec-event-content">
						<h3 class="twec-event-title"><a href="<?php echo esc_url( get_permalink() ); ?>"><?php echo esc_html( get_the_title() ); ?></a></h3>
							...
							<?php if ( $start_date ) : ?>
								<span class="twec-event-time">
									...
									if ( $all_day ) {
										...
										echo ' ';
										esc_html_e( '(All Day)', 'planit-event-manager' );
									} else {
						...
						<?php if ( has_excerpt() ) : ?>
							<div class="twec-event-excerpt"><?php echo wp_kses_post( get_the_excerpt() ); ?></div>
						<?php endif; ?>
						<?php
						if ( ! empty( $twec_list_show_ticket_ctas ) && class_exists( 'TWEC_WooCommerce', false ) ) {
